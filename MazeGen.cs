...
{
	private PackedScene Blocks = (PackedScene)GD.Load("res://Blocks.tscn");
	public bool[,] visited;
	public bool[,] hwalls; 
	public bool[,] vwalls; 
	public int size = 8;
...
//		return (int) Random.Range(0.0f, (float)size - 0.001f);
		return RNG.RandiRange(0, size-1);
	}
...
//		GD.Print("visiting: " + unvisited.ToString() + " : " + s);
	}
		...
		for (int i = 0; i < size+1; i++)
		{
			for (int j = 0; j < size+1; j++)
			{
				...
				if( hwalls[i,j] && i < size){
					Spatial block = (Spatial)Blocks.Instance();
					block.Translation = new Vector3(i*scale, 0, j*scale-scale/2);
					block.Scale = new Vector3(s, s/2, s/2);
					AddChild(block);
				}
				if( vwalls[i,j] && j < size ){
					Spatial block = (Spatial)Blocks.Instance();
					block.Translation = new Vector3(i*scale-scale/2, 0, j*scale);
					block.Scale = new Vector3(s/2, s/2, s);
					AddChild(block);
				}
			}
		}
